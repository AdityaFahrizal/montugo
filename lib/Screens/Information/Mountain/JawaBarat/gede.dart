...
class Gede extends StatefulWidget {
  const Gede({super.key});

  @override
  _GedeState createState() => _GedeState();
}

class _GedeState extends State<Gede> {
  final DocumentReference GedeRef =
      FirebaseFirestore.instance.collection('gunung').doc('gede');
  @override
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
      stream: GedeRef.snapshots(),
                ...
                child: FlutterMap(
                  options: MapOptions(
                    initialCenter: LatLng(
                        data['latitude'] ?? -6.78, data['longitude'] ?? 106.97),
                    initialZoom: 13.0,
                  ),
                  children: [
                    TileLayer(
                    ...
                    MarkerLayer(
                      markers: [
                        Marker(
                            width: 80.0,
                            height: 80.0,
                            point: LatLng(data['latitude'] ?? -6.78,
                                data['longitude'] ?? 106.97),
