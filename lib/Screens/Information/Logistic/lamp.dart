...
class Lampu extends StatefulWidget {
  ...
  @override
  _LampuState createState() => _LampuState();
}

class _LampuState extends State<Lampu> {
  final DocumentReference headlampRef =
      FirebaseFirestore.instance.collection('barang').doc('headlamp');
  @override
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
      stream: headlampRef.snapshots(),
              ...
              Column(
                children: [
                  ...
                  infoItem(FontAwesomeIcons.bolt,
                      "Sumber Daya: ${data['daya'] ?? '-'}"),
                  infoItem(FontAwesomeIcons.weightHanging,
                      "Berat: ${data['berat'] ?? '-'}"),
                  infoItem(FontAwesomeIcons.solidLightbulb,
                      "Cahaya: ${data['cahaya'] ?? '-'}"),
                ],
              ),
              const SizedBox(height: 20),
